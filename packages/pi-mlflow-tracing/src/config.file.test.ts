...
test("reads-global-scope", () => {
  const env = loadFileEnv({
    agentDir: AGENT,
    cwd: CWD,
    ...stubFs({ [GLOBAL]: JSON.stringify({ PI_MLFLOW_ENABLE: "on" }) }),
  });
  assert.equal(env.PI_MLFLOW_ENABLE, "on");
});
...
test("reads-project-scope", () => {
  const env = loadFileEnv({
    agentDir: AGENT,
    cwd: CWD,
    ...stubFs({ [PROJECT]: JSON.stringify({ PI_MLFLOW_ENDPOINT: "http://localhost:24317/mlflow-otlp/v1/traces" }) }),
  });
  assert.equal(env.PI_MLFLOW_ENDPOINT, "http://localhost:24317/mlflow-otlp/v1/traces");
});
...
test("project-overrides-global", () => {
  const env = loadFileEnv({
    ...
    cwd: CWD,
    ...stubFs({
      [GLOBAL]: JSON.stringify({ PI_MLFLOW_ENABLE: "off", PI_MLFLOW_EXPERIMENT: "global" }),
      [PROJECT]: JSON.stringify({ PI_MLFLOW_ENABLE: "on" }),
    }),
  });
  // Shared key: project wins. Global-only key: passes through.
  assert.equal(env.PI_MLFLOW_ENABLE, "on");
  assert.equal(env.PI_MLFLOW_EXPERIMENT, "global");
});
...
test("coerces-scalars-and-drops-objects", () => {
  ...
  assert.equal(env.EDGE_PORT, "1000");
  assert.equal(env.PI_MLFLOW_ENABLE, "true");
...
test("merge-env-lets-environment-win", () => {
  const fileEnv = { PI_MLFLOW_ENABLE: "on", PI_MLFLOW_EXPERIMENT: "from-file" };
  const merged = mergeEnv(fileEnv, { PI_MLFLOW_EXPERIMENT: "from-env" });
  // Environment wins for a shared key; file-only key passes through.
  assert.equal(merged.PI_MLFLOW_EXPERIMENT, "from-env");
  assert.equal(merged.PI_MLFLOW_ENABLE, "on");
});
