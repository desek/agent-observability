 ...
 *
 * Why: the extension's whole configuration surface is the PI_MLFLOW_*
 * environment variables (see config.env.ts), but a process
 ...
 * names the extension already understands (PI_MLFLOW_ENABLE, PI_MLFLOW_*), so the file
...
function readOne(
  ...
  try {
    ...
    if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
      ...
      console.error(
        `pi-mlflow-tracing: ${CONFIG_FILE_NAME} at ${path} is not a JSON object; ignoring it.`,
      );
      return out;
    }
    ...
    console.error(`pi-mlflow-tracing: failed to read ${CONFIG_FILE_NAME} at ${path}: ${err}`);
  }
  return out;
}
